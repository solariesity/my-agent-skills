...
This skill lives inside the [`my-agent-skills`](https://github.com/solariesity/my-agent-skills) monorepo. Clone that repository and enter the paper-review directory:

```bash
git clone https://github.com/solariesity/my-agent-skills.git
cd my-agent-skills/paper-review
```

### Install Only the Skill

If you only want to install paper-review as a skill in an agent environment, use a temporary clone:
...
mkdir -p ~/.codex/skills/paper-review
cp /tmp/my-agent-skills/paper-review/SKILL.md ~/.codex/skills/paper-review/

# 3. Remove the temporary directory
rm -rf /tmp/my-agent-skills
```

On Windows, use `C:\Users\<username>\.codex\skills\paper-review\` as the target path.
