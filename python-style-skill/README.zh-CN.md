...
这个 skill 存放在 [`my-agent-skills`](https://github.com/solariesity/my-agent-skills) 集合仓库中。克隆该仓库并进入 python-style-skill 目录：
...
如果你只想把 python-style-skill 当作 skill 安装到 Agent 环境，可以使用临时克隆的方式：
...
Windows 上对应路径是 `C:\Users\<用户名>\.codex\skills\python-style-skill\`。
