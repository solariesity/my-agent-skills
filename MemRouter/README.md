...
MemRouter is a skill and local tooling layer for AI agents that need structured, inspectable, and recallable memory. It classifies candidate details, routes them to predictable file paths by user, project, type, and date, and recalls them from the same scope later.
...
MemRouter supports these canonical types:
...
Default vault-relative root is `MemRouter/`. Typical paths:
...
MemRouter follows semantic versioning in spirit. `v0.5.0` means the routing model, write semantics, recall boundaries, and heuristic decision layer are stable enough for real use, but decision quality and recall quality still have room to improve before a `1.0.0` release.
