...
If the workspace already has legacy memory files, migrate durable memory into MemRouter before normal day-to-day use.
...
- extract and re-route stable facts, preferences, project facts, project decisions, and useful session summaries into MemRouter
- do not blindly import todos, reminders, diaries, or raw chat transcripts; either keep them external or summarize them first
- use normal MemRouter classification and dedupe rules during migration so imported memory lands in the same canonical files as future memory
- after migration, treat MemRouter as the single source of truth for durable memory
- do not keep writing durable memory to both legacy memory files and MemRouter

After migration, rewrite the legacy `MEMORY.md` into a routing stub instead of a content store.
That stub should say that the workspace now uses MemRouter, point to the canonical memory root, list the route families, and note which operational files still live outside MemRouter.
...
Do not persist passwords, secrets, API keys, access tokens, recovery codes, or other private credentials in MemRouter.
...
- user memory -> `MemRouter/users/<user_id>/`
- project memory -> `MemRouter/projects/<project_slug>/`
- project sessions -> `MemRouter/projects/<project_slug>/sessions/`
- user sessions -> `MemRouter/sessions/<user_id>/`
- inbox memory -> `MemRouter/inbox/`
...
- After migration, treat MemRouter as the single source of truth for durable memory.
- Do not keep dual-writing durable memory to both legacy files and MemRouter.
