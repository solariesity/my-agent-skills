...
1. If the content contains a password, secret, API key, token, recovery code, or other credential, do not persist it in memory-router.
...
- rules such as "store mechanism in memory-router, keep data in local files"
...
Keep those external, or summarize only their durable conclusions before routing them into memory-router.
