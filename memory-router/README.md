...
Current version: [v0.5.1](VERSION).

memory-router follows semantic versioning in spirit. `v0.5.1` means the routing model, write semantics, recall boundaries, and heuristic decision layer are stable enough for real use, but decision quality and recall quality still have room to improve before a `1.0.0` release.
