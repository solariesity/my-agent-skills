...
This skill lives inside the [`my-agent-skills`](https://github.com/solariesity/my-agent-skills) monorepo. Clone that repository and enter the py-style-skill directory:

```bash
git clone https://github.com/solariesity/my-agent-skills.git
cd my-agent-skills/py-style-skill
```

### Install Only the Skill

If you only want to install py-style-skill as a skill in an agent environment, use a temporary clone:
...
mkdir -p ~/.codex/skills/py-style-skill
cp /tmp/my-agent-skills/py-style-skill/SKILL.md ~/.codex/skills/py-style-skill/

# 3. Remove the temporary directory
rm -rf /tmp/my-agent-skills
```

On Windows, use `C:\Users\<username>\.codex\skills\py-style-skill\` as the target path.
