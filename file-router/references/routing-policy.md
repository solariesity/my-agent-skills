...
Use this file when you need the exact route model that file-router applies.
...
file-router resolves storage from a combination of:
...
file-router places general projects under `files/Dev/Tools/<project>/` by default.
...
When explicit route metadata is missing, file-router uses practical heuristics:
...
file-router sanitizes unsafe file names:
...
When a collision occurs and `rename` mode is active, file-router appends `_copyNN`.
